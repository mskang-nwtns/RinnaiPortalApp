 ...
 * 수동입력 필요
 * row 클릭시 상세 화면 확인 필요
 * 출하 관련 Activity -> 제품상차 로 변경
 */
public class WmsMenu08Activity extends BaseActivity implements AdapterView.OnItemClickListener{
...
//							CommonValue.agencyOrderBarcode = agencyOrderBarcode;
							...
							if(agencyOrderBarcodeList.contains(value)) {
								showRinnaiDialog(WmsMenu08Activity.this, getString(R.string.msg_title_noti), "이미 처리된 지시입니다.");
							} else {

//								agencyOrderBarcodeList.add(agencyOrderBarcode);
								CommonValue.Act_8_agencyOrderBarcodeList = agencyOrderBarcodeList;
...
//							CommonValue.agencyOrderBarcode = agencyOrderBarcode;
						...
						showRinnaiDialog(WmsMenu08Activity.this, getString(R.string.msg_title_noti), getString(R.string.msg_order_report_barcode_not_validation));
...
//		getWindow().addFlags(WindowManager.LayoutParams.FLAG_SECURE);
		...
		btnReadingBarcode.setOnClickListener(WmsMenu08Activity.this);

		//위치 모델명 형식 수 량
		registerReceiver();
		btnSubmit.setOnClickListener(WmsMenu08Activity.this);
		btnClear.setOnClickListener(WmsMenu08Activity.this);
		helper = new MySQLiteOpenHelper(
				WmsMenu08Activity.this,  // 현재 화면의 제어권자
		...
		if(CommonValue.Act_8_totalInstructions > 0) {
			totalInstructions = CommonValue.Act_8_totalInstructions;
		}

		if(CommonValue.Act_8_totalOperations > 0) {
			totalOperations = CommonValue.Act_8_totalOperations;
		}

		if(CommonValue.Act_8_totalSuccessCount > 0) {
			totalSuccessCount = CommonValue.Act_8_totalSuccessCount;
		}

		if(CommonValue.Act_8_totalRemainingCount > 0) {
			totalRemainingCount = CommonValue.Act_8_totalRemainingCount;
		}
		...
		if(CommonValue.Act_8_agencyOrderBarcodeList != null) {
			agencyOrderBarcodeList = CommonValue.Act_8_agencyOrderBarcodeList;
		}
		...
		lvReadProductBarcode.setOnItemClickListener(WmsMenu08Activity.this);
	...
	private void getOrderReport(String orderReport) {
		...
		if(!networkConnecting) {
			...
			String httpHost = HttpClient.getCurrentSsid(WmsMenu08Activity.this);
			...
			showProgress(WmsMenu08Activity.this);
...
//		no = tmpCarItem + "-" + tmpTrsNo;
	...
	private void readProductBarcode(String productBarcode) {
		...
		etReadingBarcode.setText(productBarcode);

		if(agencyOrderBarcodeList.size() == 0) {

			showRinnaiDialog(WmsMenu08Activity.this, getString(R.string.msg_title_noti), "c");
			long[] pattern = {2000, 50, 2000, 50}; // 1초 진동, 0.05초 대기, 1초 진동, 0.05초 대기
			if (Build.VERSION.SDK_INT >= 26) {
				((Vibrator) getSystemService(VIBRATOR_SERVICE)).vibrate(VibrationEffect.createOneShot(2000,150));
			} else {
		...
		if(adapter == null) {
			...
			adapter = new WmsMenu08ListAdapter(WmsMenu08Activity.this, readProductBarcodeList);
		...
		String selectQuery = String.format("SELECT %s,%s,%s,%s,%s  FROM %s WHERE %s = '%s' ;",
				...
				CommonValue.SQLITE_DB_TABLE_NAME_AGENCY_ORDER_READING_INFO,
				CommonValue.SQLITE_DB_TABLE_AGENCY_ORDER_FILED_NAME_BARCODE ,
				productBarcode);
		...
		if(count == 0) {
			...
			selectQuery = String.format("SELECT SUM(%s) AS %s, SUM(%s) AS %s FROM %s WHERE %s in ( %s ) AND %s = '%s' GROUP BY %s",
					...
					CommonValue.SQLITE_DB_TABLE_AGENCY_ORDER_FILED_NAME_OPERATIONS,
					CommonValue.SQLITE_DB_TABLE_NAME_AGENCY_ORDER_INFO,
					CommonValue.SQLITE_DB_TABLE_AGNECY_ORDER_FILED_NAME_AGENCY_ORDER_BARCODE,
					tmpBarcode,
					CommonValue.SQLITE_DB_TABLE_COMMON_FILED_NAME_ITEM_CODE,
					itemCode,
			...
			if (instructions == 0 && operations == 00) {
				showRinnaiDialog(WmsMenu08Activity.this, getString(R.string.msg_title_noti), "해당 지시내역이 아닙니다.");
				long[] pattern = {2000, 50, 2000, 50}; // 1초 진동, 0.05초 대기, 1초 진동, 0.05초 대기
				if (Build.VERSION.SDK_INT >= 26) {
					((Vibrator) getSystemService(VIBRATOR_SERVICE)).vibrate(VibrationEffect.createOneShot(2000,150));
				} else {
				...
				if(count > 0 ) {
					...
					totalOperations += 1;
					tvTotalOperations.setText(String.format("%d", totalOperations));
					CommonValue.Act_8_totalOperations = totalOperations;
					db.execSQL(query);
				...
				showRinnaiDialog(WmsMenu08Activity.this, getString(R.string.msg_title_noti), "작업이 완료된 제품입니다.");
				long[] pattern = {2000, 50, 2000, 50}; // 1초 진동, 0.05초 대기, 1초 진동, 0.05초 대기
				if (Build.VERSION.SDK_INT >= 26) {
					((Vibrator) getSystemService(VIBRATOR_SERVICE)).vibrate(VibrationEffect.createOneShot(2000,150));
				} else {
			...
			AlertDialog.Builder alert_confirm = new AlertDialog.Builder(WmsMenu08Activity.this);
...
//                            i
					...
					if(agencyBarcodeResultList.size() == 0) {
						showRinnaiDialog(WmsMenu08Activity.this, getString(R.string.msg_title_noti), getString(R.string.msg_order_report_barcode_not_result));
					}
					...
					CommonValue.Act_8_totalInstructions = totalInstructions;
					CommonValue.Act_8_totalOperations = totalOperations;
					CommonValue.Act_8_totalSuccessCount = totalSuccessCount;
					CommonValue.Act_8_totalRemainingCount = totalRemainingCount;
					...
					showRinnaiDialog(WmsMenu08Activity.this, getString(R.string.msg_title_noti), String.format(" 총 %d건 중 %d건 전송에 성공하였습니다.", count, success ));
					...
					if(null == suspend.getMemo()) {
						...
						showRinnaiDialog(WmsMenu08Activity.this, getString(R.string.msg_title_noti), message, path);
						long[] pattern = {2000, 50, 2000, 50}; // 1초 진동, 0.05초 대기, 1초 진동, 0.05초 대기
						if (Build.VERSION.SDK_INT >= 26) {
							((Vibrator) getSystemService(VIBRATOR_SERVICE)).vibrate(VibrationEffect.createOneShot(2000,150));
						} else {
	...
	public void onClick(View v) {
		...
		if (id == btnSubmit.getId()) {
			AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(WmsMenu08Activity.this);
			// 제목셋팅
...
//									} else {
										...
										String url = String.format("%s/%s/%s",CommonValue.HTTP_HOST, CommonValue.HTTP_WMS, CommonValue.HTTP_AGENCY_ORDER);
										showProgress(WmsMenu08Activity.this);
										networkConnecting = true;
										HttpClient.post(url, WmsMenu08Activity.this, jsonArray.toString());
...
//			String value = new String(scanvalue);
	...
	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		...
		if("0".equals(info.getQty())) {
			showRinnaiDialog(WmsMenu08Activity.this, getString(R.string.msg_title_noti), "작업된 수량이 없습니다.");
		} else {
			...
			WmsMenuReadingInfoDialog dialog = new WmsMenuReadingInfoDialog(WmsMenu08Activity.this, listReadingInfo, modelName);
			dialog.show();
		}
	}
}
