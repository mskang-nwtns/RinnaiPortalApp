...
public class AgencyFragmentFirst extends Fragment implements View.OnClickListener, View.OnTouchListener {
	...
	public static Fragment newInstance(Context context, int position, ArrayList<SensorData> sensorDataList) {
		...
		return Fragment.instantiate(context, AgencyFragmentFirst.class.getName(), b);
	}
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		convertView = inflater.inflate(R.layout.act_agency_first_page, container, false);
		...
		switch (event.getAction()) {
			...
			case MotionEvent.ACTION_UP:
				...
				if(id == menu01.getId()) {
					...
					if(isPressed) {
						intent = new Intent(getActivity(), AgencyMenu01Activity.class);
						((AgencyActivity)getActivity()).setLog("AgencyMenu01Activity");
					}
				} else if(id == menu02.getId()) {

					menu02.buttonClick(false);
					if(isPressed) {
						intent = new Intent(getActivity(), AgencyMenu02Activity.class);
						((AgencyActivity)getActivity()).setLog("AgencyMenu02Activity");
					}
				} else if(id == menu03.getId()) {
					menu03.buttonClick(false);
					if(isPressed) {
						intent = new Intent(getActivity(), AgencyMenu03Activity.class);
						((AgencyActivity)getActivity()).setLog("AgencyMenu03Activity");
					}
				} else if(id == menu04.getId()) {
					menu04.buttonClick(false);
					if(isPressed) {
						intent = new Intent(getActivity(), AgencyMenu04Activity.class);
						((AgencyActivity)getActivity()).setLog("AgencyMenu04Activity");
					}
				} else if(id == menu05.getId()) {
					menu05.buttonClick(false);
					if(isPressed) {
						intent = new Intent(getActivity(), AgencyMenu05Activity.class);
						((AgencyActivity)getActivity()).setLog("AgencyMenu05Activity");
					}
				} else if(id == menu07.getId()) {
					...
					if(isPressed) {
						...
						((AgencyActivity)getActivity()).setLog("AgencyMenu06Activity");
					}
				}
				if(intent != null) {
