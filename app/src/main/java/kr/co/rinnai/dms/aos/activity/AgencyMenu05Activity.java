...
public class AgencyMenu05Activity extends BaseActivity  implements AdapterView.OnItemSelectedListener, AdapterView.OnItemClickListener {
	...
	private TextView tvStockInchon, tvStockGajwa, tvStockDaejeon, tvStockGwangju, tvStockGyeongsan, tvStockBusan, tvOrderStockQ, tvNOutQty;
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		setContentView(R.layout.activity_agency_menu_05);
		...
		spMaster.setOnItemSelectedListener(AgencyMenu05Activity.this);
		spSub.setOnItemSelectedListener(AgencyMenu05Activity.this);

		lvStockOrderable.setOnItemClickListener(AgencyMenu05Activity.this);

		btnCalendar = (RelativeLayout) findViewById(R.id.rl_agency_05_date);
        btnCalendar.setOnClickListener(AgencyMenu05Activity.this);
		...
		helper = new MySQLiteOpenHelper(
				AgencyMenu05Activity.this,  // 현재 화면의 제어권자
		...
		if (null != responseData) {
			...
			if ("OK".equals(responseData.getResultMessage())) {
				...
				if ("getCategorization".equals(responseData.getResultType())) {
					...
					masterAdapter = new CategorySpinnerAdapter(AgencyMenu05Activity.this, categori.getMaster());
					spMaster.setAdapter(masterAdapter);
					masterAdapter.notifyDataSetChanged();

					subAdapter = new CategorySpinnerAdapter(AgencyMenu05Activity.this, categori.getSub().get("-1"));
...
//					tvStockTotal.setText(String.format("%,d", info.getInchon() + info.getGajwa() + info.getDaejeon() + info.getGwangju() + info.getGyeongsan() + info.getBusan()));
				...
				showRinnaiDialog(AgencyMenu05Activity.this, getString(R.string.msg_title_noti),responseData.getResultMessage());
			}
		}
	}
...
//
    ...
    public void onClick(View v) {
        if(rinnaiReceivedProductDialog == null) {
			rinnaiReceivedProductDialog = new RinnaiCalendarDialog(AgencyMenu05Activity.this);
		}
...
//			    searchDate.replace("")
            ...
            String url = String.format("%s/%s/%s/%s/%s/%s", CommonValue.HTTP_HOST, CommonValue.HTTP_AOS, CommonValue.HTTP_STOCK, tmpDate, masterCode, subCode);
